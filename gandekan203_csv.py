...
df = pd.read_csv('Gandekan_20250619_203.xlsx - Sheet1.csv', skiprows=1)
...
df_cleaned.to_csv('akses_cleaned203.csv', index=False)
df_cleaned.to_excel('akses_cleaned203.xlsx', index=False)
...
plt.tight_layout()
plt.savefig('akses_per_tanggal203.png')
plt.show()
...
plt.ylabel("Jam")
plt.tight_layout()
plt.savefig('heatmap_jam_vs_tanggal203.png')
plt.show()
...
plt.ylabel('')
plt.tight_layout()
plt.savefig('distribusi_card_type203.png')
plt.show()
...
summary_writer = pd.ExcelWriter('laporan_ringkasan_akses203.xlsx', engine='xlsxwriter')
