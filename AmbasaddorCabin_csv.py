...
folder_path = "./data_ambasaddor"

csv_files = [
    f for f in os.listdir(folder_path)
    if f.lower().endswith('.csv')
    and 'ambasaddor' in f.lower()
...
per_file_writer = pd.ExcelWriter("File-AmbasaddorCabin.xlsx", engine="xlsxwriter")
...
for sumber_file, df_per_file in df_with_source.groupby("sumber_file"):
    ...
    sheet_name = sumber_file.replace(".csv", "").replace("Ambasaddor_", "Kamar_")[:31]
...
print("📁 File 'File-AmbasaddorCabin.xlsx' berhasil dibuat!")
