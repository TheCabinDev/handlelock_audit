...
df = pd.read_csv('Gandekan_20250619_206.xlsx - Sheet1.csv', skiprows=1)
...
df_cleaned.to_csv('akses_cleaned206.csv', index=False)
df_cleaned.to_excel('akses_cleaned206.xlsx', index=False)
...
plt.tight_layout()
plt.savefig('akses_per_tanggal206.png')
plt.show()
...
plt.ylabel("Jam")
plt.tight_layout()
plt.savefig('heatmap_jam_vs_tanggal206.png')
plt.show()
...
plt.ylabel('')
plt.tight_layout()
plt.savefig('distribusi_card_type206.png')
plt.show()
...
summary_writer = pd.ExcelWriter('laporan_ringkasan_akses206.xlsx', engine='xlsxwriter')
