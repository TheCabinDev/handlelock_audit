...
folder_path = "./data_sidoarum"

csv_files = [
    f for f in os.listdir(folder_path)
    if f.lower().endswith('.csv')
    and 'sidoarum' in f.lower()
...
per_file_writer = pd.ExcelWriter("File-SidoarumCabin.xlsx", engine="xlsxwriter")
...
for sumber_file, df_per_file in df_with_source.groupby("sumber_file"):
    ...
    sheet_name = sumber_file.replace(".csv", "").replace("Sidoarum_", "Kamar_")[:31]
...
print("📁 File 'File-SidoarumCabin.xlsx' berhasil dibuat!")
