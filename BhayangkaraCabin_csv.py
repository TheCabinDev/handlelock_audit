...
folder_path = "./data_bhayangkara"

csv_files = [
    f for f in os.listdir(folder_path)
    if f.lower().endswith('.csv')
    and 'bhayangkara' in f.lower()
...
per_file_writer = pd.ExcelWriter("File-BhayangkaraCabin.xlsx", engine="xlsxwriter")
...
for sumber_file, df_per_file in df_with_source.groupby("sumber_file"):
    ...
    sheet_name = sumber_file.replace(".csv", "").replace("Bhayangkara_", "Kamar_")[:31]
...
print("📁 File 'File-BhayangkaraCabin.xlsx' berhasil dibuat!")
