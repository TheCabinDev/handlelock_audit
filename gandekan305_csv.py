...
df = pd.read_csv('Gandekan_20250619_305.xlsx - Sheet1.csv', skiprows=1)
...
df_cleaned.to_csv('akses_cleaned305.csv', index=False)
df_cleaned.to_excel('akses_cleaned305.xlsx', index=False)
...
plt.tight_layout()
plt.savefig('akses_per_tanggal305.png')
plt.show()
...
plt.ylabel("Jam")
plt.tight_layout()
plt.savefig('heatmap_jam_vs_tanggal305.png')
plt.show()
...
plt.ylabel('')
plt.tight_layout()
plt.savefig('distribusi_card_type305.png')
plt.show()
...
summary_writer = pd.ExcelWriter('laporan_ringkasan_akses305.xlsx', engine='xlsxwriter')
